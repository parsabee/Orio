...
class Transformation(object):
    ...
    def __init__(self, stmt, devProps, targs, tinfo=None):
        ...
        if self.tinfo is not None and self.streamCount > 1:
            ...
            if len(ivarListLengths) > 1:
                raise Exception((
                    'orio.module.loop.submodule.cuda.transformation: streaming for different-length arrays is not supported'))
        ...
        errorChecks += [
            ...
            IfStmt(BinOpExp(IdentExp('cudaSuccess'), errIdent, BinOpExp.NE),
                   ExpStmt(FunCallExp(IdentExp('printf'),
                                      [StringLitExp('CUDA runtime error: %s@'),
                                       FunCallExp(IdentExp('cudaGetErrorString'), [errIdent])])))
        ]
        # errorChecks += [
    ...
    def createStreamDecls(self):
        ...
        self.newstmts['streamAllocs'] = [
            ...
            VarDecl('int', [self.cs['istream'], self.cs['soffset']] + (
                [self.cs['boffset']] if self.model['isReduction'] else [])),
            ...
            ForStmt(BinOpExp(self.cs['istream'], self.cs['int0'], BinOpExp.EQ_ASGN),
                    ...
                    ExpStmt(FunCallExp(IdentExp('cudaStreamCreate'),
                                       [UnaryExp(ArrayRefExp(IdentExp('stream'), self.cs['istream']),
                                                 UnaryExp.ADDRESSOF)]))),
            VarDeclInit('int', self.cs['chunklen'],
                        BinOpExp(self.model['inputsize'], self.cs['nstreams'], BinOpExp.DIV)),
            VarDeclInit('int', self.cs['chunkrem'],
                        BinOpExp(self.model['inputsize'], self.cs['nstreams'], BinOpExp.MOD)),
        ]
        ...
        deallocs = [
            ForStmt(BinOpExp(self.cs['istream'], self.cs['int0'], BinOpExp.EQ_ASGN),
                    BinOpExp(self.cs['istream'], self.cs['nstreams'], BinOpExp.LE),
                    UnaryExp(self.cs['istream'], UnaryExp.POST_INC),
                    ExpStmt(FunCallExp(IdentExp('cudaStreamDestroy'),
    ...
    def createMallocs(self):
        ...
        if self.tinfo is None:  # inference mode
            mallocs += [VarDeclInit('int', self.cs['nbytes'],
                                    BinOpExp(self.model['inputsize'], self.cs['sizeofDbl'], BinOpExp.MUL))]
        ...
        for aid, daid in self.model['arrays']:
            if self.tinfo is None:
                ...
                if len(aidtinfo) == 0:
                    raise Exception(
                        'orio.module.loop.submodule.cuda.transformation: %s: unknown input variable argument: "%s"' % aid)
                else:
                    aidtinfo = aidtinfo[0]
                aidbytes = BinOpExp(IdentExp(aidtinfo[4][0]), FunCallExp(IdentExp('sizeof'), [IdentExp(aidtinfo[2])]),
                                    BinOpExp.MUL)
            mallocs += [
            ...
            for raid, draid in res_array_ids:
                if self.streamCount > 1:
                    ...
                    if self.tinfo is None:
                        ...
                        raidbytes = BinOpExp(IdentExp(raidtinfo[4][0]),
                                             FunCallExp(IdentExp('sizeof'), [IdentExp(raidtinfo[2])]), BinOpExp.MUL)
                    d2hcopys += [
                        ExpStmt(FunCallExp(IdentExp('cudaMemcpy'),
        ...
        if self.streamCount > 1 and not self.model['isReduction']:
            ...
            d2hcopys += [ForStmt(BinOpExp(self.cs['istream'], self.cs['int0'], BinOpExp.EQ_ASGN),
                                 BinOpExp(self.cs['istream'], self.cs['nstreams'], BinOpExp.LE),
                                 UnaryExp(self.cs['istream'], UnaryExp.POST_INC),
                                 ExpStmt(FunCallExp(IdentExp('cudaStreamSynchronize'),
                                                    [ArrayRefExp(IdentExp('stream'), self.cs['istream'])])))]
    ...
    def createKernelCalls(self):
        ...
        if self.streamCount == 1:
            ...
            kernell_call = ExpStmt(FunCallExp(IdentExp(
                self.state['dev_kernel_name'] + '<<<blks4chunk,dimBlock,0,stream[' + str(self.cs['istream']) + ']>>>'),
                args))
            kernell_callrem = ExpStmt(FunCallExp(IdentExp(
                self.state['dev_kernel_name'] + '<<<blks4chunk,dimBlock,0,stream[' + str(self.cs['istream']) + ']>>>'),
                argsrem))
            kernell_calls += [
                # calc blocks per stream
                VarDeclInit('int', self.cs['blks4chunk'],
                            BinOpExp(self.cs['gridx'], self.cs['nstreams'], BinOpExp.DIV)),
                IfStmt(BinOpExp(BinOpExp(self.cs['gridx'], self.cs['nstreams'], BinOpExp.MOD), self.cs['int0'],
                                BinOpExp.NE),
            ...
            boffsets = []
            if self.model['isReduction']:
                kernell_calls += [VarDeclInit('int', self.cs['blks4chunks'],
                                              BinOpExp(self.cs['blks4chunk'], self.cs['nstreams'], BinOpExp.MUL))]
            ...
            kernell_calls += [
                ...
                IfStmt(BinOpExp(self.cs['chunkrem'], self.cs['int0'], BinOpExp.NE),
                       CompStmt(self.state['calc_offset'] + boffsets + [kernell_callrem] +
                                ([ExpStmt(UnaryExp(self.cs['blks4chunks'], UnaryExp.POST_INC))] if self.model[
                                    'isReduction'] else [])))
            ]
        ...
        if self.model['isReduction']:
            ...
            bodyStmts = [
                ...
                ExpStmt(BinOpExp(stageBlocksIdent,
                                 BinOpExp(ParenthExp(
                                     BinOpExp(stageBlocksIdent, NumLitExp(str(self.threadCount - 1), NumLitExp.INT),
                                              BinOpExp.ADD)),
                ...
                ExpStmt(FunCallExp(IdentExp(
                    self.state['dev_redkern_name'] + '<<<' + stageBlocks + ',' + str(self.threadCount) + '>>>'),
    ...
    def transform(self):
        ...
        index_id, lbound_exp, ubound_exp, stride_exp, loop_body = orio.module.loop.ast_lib.forloop_lib.ForLoopLib().extractForLoopInfo(
            self.stmt)
        ...
        for lb_stmt in lb_stmts:
            if isinstance(lb_stmt, ForStmt):
                index_id2, lbound_exp2, ubound_exp2, stride_exp2, loop_body2 = orio.module.loop.ast_lib.forloop_lib.ForLoopLib().extractForLoopInfo(
                    lb_stmt)
                nestedLoop = True
                ...
                if len(lb_stmts) == 1:
                    self.canBe2D = True
                    if isinstance(loop_body2, CompStmt):
                        if len(loop_body2.stmts) == 1 and isinstance(loop_body2.stmts, ForStmt):
                            self.canBe3D = True
                            index_id3, lbound_exp3, ubound_exp3, stride_exp3, _ = orio.module.loop.ast_lib.forloop_lib.ForLoopLib().extractForLoopInfo(
                                loop_body2.stmts)
                            ktempints += [index_id3]  # declare inner index
                            indices += [index_id3.name]
        # abbreviations
            ...
            def collectIntIds(n):
                ...
                if isinstance(n, BinOpExp):
                    if isinstance(n.lhs, IdentExp) and n.lhs.name in inferredInts:
                        if n.op_type != BinOpExp.EQ_ASGN and n.op_type != BinOpExp.LT and n.op_type != BinOpExp.LE and n.op_type != BinOpExp.MOD:  # and so forth depending on the typing rules
                            return []
                        else:
                            return loop_lib.collectNode(collectIdents, n.rhs)
                    else:
                        return loop_lib.collectNode(collectIntIds, n.lhs)
                else:
                    return []
        ...
        collectArrayIdents = lambda n: [n.exp.name] if (
                isinstance(n, ArrayRefExp) and isinstance(n.exp, IdentExp)) else []
        ...
        if g.Globals().validationMode and not g.Globals().executedOriginal:
            ...
            testOrigOutput = [
                VarDeclInit('FILE*', printFpIdent,
                            FunCallExp(IdentExp('fopen'), [StringLitExp('origexec.out'), StringLitExp('w')])),
                original
            ]
            ...
            for var in self.model['lhss']:
                if var in array_ids:
                    bodyStmts += [ExpStmt(FunCallExp(IdentExp('fprintf'),
                                                     [printFpIdent, StringLitExp("\'" + var + "[%d]\',%f; "), index_id,
                                                      ArrayRefExp(IdentExp(var), index_id)])
                                          )]
                else:
                    testOrigOutput += [ExpStmt(FunCallExp(IdentExp('fprintf'),
                                                          [printFpIdent, StringLitExp("\'" + var + "\',%f"),
                                                           IdentExp(var)]))]
        ...
        int_ids_pass2 = set([x for x in loop_lib.collectNode(collectIntIdsClosure(inferredInts), loop_body) if
                             x not in (indices + ubound_ids)])
        ...
        # loop_body2 = loop_lib.rewriteNode(rewriteToTid, loop_body)
        loop_body2 = loop_body
        ...
        if self.domain != None:
            ...
            domainStmts += [
                VarDeclInit('int', gridmIdent, FunCallExp(IdentExp('round'), [
                    FunCallExp(IdentExp('pow'), [self.model['inputsize'], CastExpr('double', BinOpExp(self.cs['int1'],
                                                                                                      self.cs['int3'],
                                                                                                      BinOpExp.DIV))])
                ])),
                ...
                ExpStmt(BinOpExp(ArrayRefExp(sidxIdent, self.cs['int2']), BinOpExp(gridmIdent, dofIdent, BinOpExp.MUL),
                                 BinOpExp.EQ_ASGN)),
                ExpStmt(BinOpExp(ArrayRefExp(sidxIdent, self.cs['int3']),
                                 BinOpExp(gridmIdent, BinOpExp(gridnIdent, dofIdent, BinOpExp.MUL), BinOpExp.MUL),
                                 BinOpExp.EQ_ASGN)),
                ExpStmt(BinOpExp(ArrayRefExp(sidxIdent, int4), UnaryExp(dofIdent, UnaryExp.MINUS), BinOpExp.EQ_ASGN)),
                ExpStmt(BinOpExp(ArrayRefExp(sidxIdent, int5),
                                 BinOpExp(UnaryExp(gridmIdent, UnaryExp.MINUS), dofIdent, BinOpExp.MUL),
                                 BinOpExp.EQ_ASGN)),
                ExpStmt(BinOpExp(ArrayRefExp(sidxIdent, int6), BinOpExp(UnaryExp(gridmIdent, UnaryExp.MINUS),
                                                                        BinOpExp(gridnIdent, dofIdent, BinOpExp.MUL),
                                                                        BinOpExp.MUL), BinOpExp.EQ_ASGN))
            ]
        ...
        blockIdx_x = IdentExp('blockIdx.x')
        blockSize_x = IdentExp('blockDim.x')
        gridSize_x = IdentExp('gridDim.x')
        threadIdx_x = IdentExp('threadIdx.x')

        if self.canBe2D or self.canBe3D:
            blockIdx_y = IdentExp('blockIdx.y')
            blockSize_y = IdentExp('blockDim.y')
            gridSize_y = IdentExp('gridDim.y')
            threadIdx_y = IdentExp('threadIdx.y')

        if self.canBe3D:
            blockIdx_z = IdentExp('blockIdx.z')
            blockSize_z = IdentExp('blockDim.z')
            gridSize_z = IdentExp('gridDim.z')
            threadIdx_z = IdentExp('threadIdx.z')

        gsizeIdent = IdentExp('gsize')
        tidVarDecl = VarDeclInit('const int', tidIdent,
                                 BinOpExp(BinOpExp(blockIdx_x, blockSize_x, BinOpExp.MUL),
                                          BinOpExp(threadIdx_x, IdentExp(lbound_id), BinOpExp.ADD) if str(
                                              lbound_exp) != '0' else threadIdx_x,
                                          BinOpExp.ADD))
        gsizeDecl = VarDeclInit('const int', gsizeIdent, BinOpExp(gridSize_x, blockSize_x, BinOpExp.MUL))
        ...
        cacheReads = []
        cacheWrites = []
        if self.cacheBlocks and not isinstance(loop_body2, ForStmt):
            for var in array_ids:
                ...
                sharedVarExp = ArrayRefExp(IdentExp(sharedVar), threadIdx_x)
                ...
                loop_body2 = loop_lib.rewriteNode(rrRhsExprs, loop_body2)
                ...
                if var in lhs_array_ids:
                    ...
                    loop_body2 = loop_lib.rewriteNode(rrLhsExprs, loop_body2)
        ...
        # --------------------------------------------------
        if isinstance(loop_body2, ForStmt):
            if self.unrollInner is not None and self.unrollInner > 1:
                loop_body2 = CompStmt([Pragma('unroll ' + str(self.unrollInner)), loop_body2])
            # else:
        ...
        if self.domain == None:
            ...
            kernelStmts += [
                ForStmt(
                    ...
                    CompStmt(cacheReads + [loop_body2] + cacheWrites))
            ]
        else:
            kernelStmts = [
                VarDeclInit('int', tidIdent, BinOpExp(blockIdx_x, ArrayRefExp(sidxIdent, threadIdx_x), BinOpExp.ADD))]
            kernelStmts += [
        ...
        if self.model['isReduction']:
            ...
            kernelStmts += [ExpStmt(BinOpExp(ArrayRefExp(blkdataIdent, threadIdx_x),
                                             loop_lhs_exprs[0],
                                             BinOpExp.EQ_ASGN))]
            ...
            def unrollTemplate(tc, offset):
                if offset != 0:
                    return [
                        IfStmt(BinOpExp(BinOpExp(threadIdx_x, NumLitExp(offset, NumLitExp.INT), BinOpExp.GE),
                                        BinOpExp(threadIdx_x, NumLitExp(offset + tc, NumLitExp.INT), BinOpExp.LT),
                                        BinOpExp.LAND),
                               ExpStmt(BinOpExp(ArrayRefExp(blkdataIdent, threadIdx_x),
                                                ArrayRefExp(blkdataIdent,
                                                            BinOpExp(threadIdx_x, NumLitExp(tc, NumLitExp.INT),
                                                                     BinOpExp.ADD)),
                                                BinOpExp.ASGN_ADD))),
                        ExpStmt(FunCallExp(IdentExp('__syncthreads'), []))
                    ]
                    ...
                    return [
                        IfStmt(BinOpExp(threadIdx_x, NumLitExp(tc, NumLitExp.INT), BinOpExp.LT),
                               ExpStmt(BinOpExp(ArrayRefExp(blkdataIdent, threadIdx_x),
                                                ArrayRefExp(blkdataIdent,
                                                            BinOpExp(threadIdx_x, NumLitExp(tc, NumLitExp.INT),
                                                                     BinOpExp.ADD)),
                ...
                if offset != 0:
                    return [
                        IfStmt(BinOpExp(BinOpExp(threadIdx_x, NumLitExp(offset, NumLitExp.INT), BinOpExp.GE),
                                        BinOpExp(threadIdx_x, NumLitExp(offset + tc, NumLitExp.INT), BinOpExp.LT),
                                        BinOpExp.LAND),
                               ExpStmt(FunCallExp(IdentExp(warp), [threadIdx_x, blkdataIdent])))
                    ]
                else:
                    return [
                        IfStmt(BinOpExp(threadIdx_x, NumLitExp(tc, NumLitExp.INT), BinOpExp.LT),
            ...
            reduceStmts += [
                ExpStmt(FunCallExp(IdentExp('__syncthreads'), [])),
                IfStmt(BinOpExp(threadIdx_x, self.cs['int0'], BinOpExp.EQ),
                       ExpStmt(BinOpExp(ArrayRefExp(reductsIdent, blockIdx_x), baseExpr, BinOpExp.EQ_ASGN)))
            ]
        ...
        if self.model['isReduction']:
            ...
            if tcount < warpSize:
                ...
                raise Exception(
                    "%s: thread count that is not a multiple of device warp size of %s is not recommended, detected %s."
                    % (self.__class__, warpSize, tcount))

            if warpkern32 is None:
                warpKern32Stmts = [
                    ExpStmt(BinOpExp(ArrayRefExp(reductsIdent, tidIdent),
                                     ArrayRefExp(reductsIdent,
                                                 BinOpExp(tidIdent, NumLitExp(16, NumLitExp.INT), BinOpExp.ADD)),
                                     BinOpExp.ASGN_ADD)),
                    ExpStmt(BinOpExp(ArrayRefExp(reductsIdent, tidIdent),
                                     ArrayRefExp(reductsIdent,
                                                 BinOpExp(tidIdent, NumLitExp(8, NumLitExp.INT), BinOpExp.ADD)),
                                     BinOpExp.ASGN_ADD)),
                    ExpStmt(BinOpExp(ArrayRefExp(reductsIdent, tidIdent),
                                     ArrayRefExp(reductsIdent,
                                                 BinOpExp(tidIdent, NumLitExp(4, NumLitExp.INT), BinOpExp.ADD)),
                                     BinOpExp.ASGN_ADD)),
                    ExpStmt(BinOpExp(ArrayRefExp(reductsIdent, tidIdent),
                                     ArrayRefExp(reductsIdent,
                                                 BinOpExp(tidIdent, NumLitExp(2, NumLitExp.INT), BinOpExp.ADD)),
                                     BinOpExp.ASGN_ADD)),
                    ExpStmt(BinOpExp(ArrayRefExp(reductsIdent, tidIdent),
                                     ArrayRefExp(reductsIdent,
                                                 BinOpExp(tidIdent, NumLitExp(1, NumLitExp.INT), BinOpExp.ADD)),
                                     BinOpExp.ASGN_ADD))
                ]
                warpkern32 = FunDecl(dev_warpkern32_name, 'void', ['__device__'], warpkernParams,
                                     CompStmt(warpKern32Stmts))
            if warpkern64 is None:
                warpKern64Stmts = [
                                      ExpStmt(BinOpExp(ArrayRefExp(reductsIdent, tidIdent),
                                                       ArrayRefExp(reductsIdent,
                                                                   BinOpExp(tidIdent, NumLitExp(32, NumLitExp.INT),
                                                                            BinOpExp.ADD)),
                                                       BinOpExp.ASGN_ADD)),
                                  ] + warpKern32Stmts
                warpkern64 = FunDecl(dev_warpkern64_name, 'void', ['__device__'], warpkernParams,
                                     CompStmt(warpKern64Stmts))
            g.Globals().cunit_declarations += [
                orio.module.loop.codegen.CodeGen('cuda').generator.generate(warpkern32, '', '  ') + '\n']
            g.Globals().cunit_declarations += [
                orio.module.loop.codegen.CodeGen('cuda').generator.generate(warpkern64, '', '  ') + '\n']
        # end warp reduce
        # --------------------------------------------------
        ...
        g.Globals().cunit_declarations += orio.module.loop.codegen.CodeGen('cuda').generator.generate(dev_kernel, '',
                                                                                                      '  ')
        if self.model['isReduction']:
            g.Globals().cunit_declarations += '\n' + orio.module.loop.codegen.CodeGen('cuda').generator.generate(
                dev_redkern, '', '  ')
        ...
        if g.Globals().validationMode:
            ...
            testNewOutput = [
                VarDeclInit('FILE*', printFpIdent,
                            FunCallExp(IdentExp('fopen'), [StringLitExp('newexec.out'), StringLitExp('w')])),
            ]
            bodyStmts = []
            for var in self.model['lhss']:
                if var in array_ids:
                    bodyStmts += [ExpStmt(FunCallExp(IdentExp('fprintf'),
                                                     [printFpIdent, StringLitExp("\'" + var + "[%d]\',%f; "), idxIdent,
                                                      ArrayRefExp(IdentExp(var), idxIdent)]))
                                  ]
                else:
                    testNewOutput += [ExpStmt(
                        FunCallExp(IdentExp('fprintf'),
                                   [printFpIdent, StringLitExp("\'" + var + "\',%f"), IdentExp(var)])
                    )]
            if len(bodyStmts) > 0:
                testNewOutput += [
