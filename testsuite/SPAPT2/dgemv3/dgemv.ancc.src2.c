...
/*@ begin Loop (
  ...
  transform Composite(
      ...
      openmp = (OMP1, 'omp parallel for private(iii,jjj,ii,jj,i,j)')
   )
  ...
  transform Composite(
      ...
      openmp = (OMP2, 'omp parallel for private(i)')
   )
   for (i = 0; i <= A_nrows-1; i++) 
        z[i] = t6[i]+z[i]; 
  ...
  transform Composite(
      ...
      openmp = (OMP3, 'omp parallel for private(i)')
   )
    for (i = 0; i <= A_nrows-1; i++) {
       y[i] = t2[i]+y[i]; 
       z[i] = t6[i]+z[i]; 
